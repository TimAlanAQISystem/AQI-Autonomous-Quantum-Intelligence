"""
VAD-enabled stability recorder for AudioManager

Usage:
    python run_am_stability_vad.py [seconds]

This script records audio using `AudioManager(test_mode=False)` so VAD is active.
Speak into the microphone during the run to validate speech detection.
"""
import sys
import time
import wave
...
if __name__ == '__main__':
    ...
    print(f"Recording VAD-enabled stability test for {duration} seconds... Speak clearly into the mic now.")
    am = AudioManager(test_mode=False, forward_raw=True)
    ...
    out_path = 'stability_capture_vad.wav'
    ...
    am.shutdown()
    print('VAD stability test complete')
