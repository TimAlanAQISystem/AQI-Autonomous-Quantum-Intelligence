...
# [PURIFIED] aqi_tts_engine is DEAD — all TTS now handled by OpenAI TTS in relay server.
# Set to None so any lingering function that references it will no-op safely.
aqi_tts_engine = None
...
_voice_sessions_lock = asyncio.Lock()  # [SWEEP] Thread-safe access
...
async def preload_text_to_voice(text: str, session_id: str):
    ...
    # [PURIFIED] TTS engine removed — relay server handles all TTS now
    logger.info("[PRELOAD] Skipped — TTS handled by relay server OpenAI TTS")
    return
...
async def send_hybrid_greeting(short_text: str, long_text: str, session_id: str, force: bool = False):
    ...
    # [PURIFIED] TTS engine removed — relay server handles all TTS now
    logger.info("[HYBRID] Skipped — TTS handled by relay server OpenAI TTS")
    return
...
async def send_text_to_voice(text: str, session_id: str):
    ...
    # [PURIFIED] TTS engine removed — relay server handles all TTS now
    logger.info("[TTS] Skipped — TTS handled by relay server OpenAI TTS")
    session.agent_speaking = False
    session.awaiting_user = True
...
@router.post("/aqi/internal/voice")
async def twilio_voice_webhook(request: Request):
    """
    [RENAMED] Internal Voice Webhook - Renamed to prevent conflict with control_api_fixed.py
    Use /twilio/voice in control_api_fixed.py for actual entry point.
    """
    try:
        ...
        # Read tunnel URL from active_tunnel_url.txt (maintained by cloudflared)
        try:
            import os
            tunnel_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'active_tunnel_url.txt')
            with open(tunnel_file, 'r') as f:
                tunnel_url = f.read().strip()
        except Exception:
            tunnel_url = "https://placeholder-tunnel-url.trycloudflare.com"
...
async def start_voice_session(request: Request):
    ...
    # [PURIFIED] RAM greeting disabled — relay server handles all TTS now
    # No aqi_tts_engine to populate from
